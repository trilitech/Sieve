...
**Residual note:** the cleanup goroutine has no termination signal and runs for the lifetime of the process. This is acceptable because `Server` is singleton per process; if a caller ever constructs multiple `Server` instances in a single process (e.g., in tests), they would leak goroutines. Not exploitable — purely a resource-usage consideration.
...
`internal/connectors/httpproxy/httpproxy.go:212-241`

The path is percent-decoded with `url.PathUnescape`, then split and checked segment-by-segment for `..`. Any `..` segment is rejected with 400. The decoded path is then normalized with `path.Clean` and required to start with `/`. The target URL is built with `(*url.URL).JoinPath` rather than string concatenation, so no raw path characters leak into the URL.

**Residual risk — double-encoded traversal:** an attacker can send `%252e%252e%252f` (double-encoded `..`). `url.PathUnescape` decodes once, producing the literal string `%2e%2e%2f`. The segment check sees `["%2e%2e%2f"]` — no `..` — and lets it through. If the upstream target decodes the URL a second time (rare, but possible with misconfigured reverse proxies), it could interpret the segment as parent-directory traversal. **Severity: LOW** because the attack requires a decode-twice backend, and the primary attack surface (single-encoded `..%2f`) is closed. See "New Observations" below.
...
`internal/policy/script.go:72`

The script timeout now derives from `context.Background()`, so an already-cancelled caller context no longer short-circuits the script timer. The caller's `ctx` is explicitly discarded with `_ = ctx`.

**Tradeoff noted in review:** the previous code would also abort the script if the caller's context was cancelled mid-request (e.g., agent disconnects during a long-running script). The new code waits for the full configured timeout even when the caller has given up. This is acceptable because script timeouts are bounded to 5s by default — the extra wall time is a tiny resource cost in exchange for correct timeout semantics under all caller-context states.

---

## New Observations (Re-Audit)

### LOW-1: Double-encoded path traversal residual

**File:** `internal/connectors/httpproxy/httpproxy.go:214-225`

**Scenario:** As noted under finding #7, `url.PathUnescape` only decodes once. A request with `%252e%252e%252f` passes the `..` segment check because the first decode produces `%2e%2e%2f`, not `..`. If the upstream target decodes again, the effective path becomes `../`.

**Likelihood:** LOW. Most target APIs decode once. Exploitable only if (a) the target performs a second decode, and (b) the target's own security model relies on the path being within a restricted prefix.

**Recommended fix (future hardening):** decode in a loop until the string stabilizes, then apply the `..` check:
...
This is intentionally left un-fixed in the current change set to avoid scope creep; flagged here for tracking.

### LOW-2: OAuth cleanup goroutine lifetime

**File:** `internal/web/server.go:188`

**Scenario:** `NewServer` unconditionally spawns `oauthPendingCleanupLoop` with no way to stop it. Acceptable for the process-singleton Server used in production, but tests that construct multiple `Server` instances (e.g., parallel subtests in a future `internal/web/server_test.go`) would accumulate goroutines.

**Recommended fix (future):** accept a `context.Context` in `NewServer` that the cleanup loop watches, or expose a `Close()` method on `Server`. Not required today since no test constructs multiple Servers in one process.
...
| LOW | 2 | Double-encoded path traversal residual; OAuth cleanup goroutine lifetime |
