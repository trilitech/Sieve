...
// (the operator credential introduced by spec 001-fix-security-vulns).
//
// The credential is distinct from the keyring passphrase (FR-030) and is held
// as an Argon2id verifier — salt + memory-hard derived key — rather than as
// an encrypted secret (FR-031). Preimage resistance is the protection; there
